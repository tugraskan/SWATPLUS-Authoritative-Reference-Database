...
:data:`FILE_SCHEMAS` covers all name-keyed files, and ``internal/tests/
...
Note: the authoritative reference files were bootstrapped from the official
SWAT+ Editor reference dataset (``swatplus_datasets.sqlite``) using the
Editor's own file writers. The bootstrap snapshot matched the Editor byte for
byte; accepted changes may differ until the next upstream sync. Seven files
that the official dataset does not carry -- the manure databases, metals,
salt, transplant, puddle, and the pathogens example set -- are retained as
supplemental and tracked in ``internal/metadata/external_sources.json``. The
per-file source table, dataset version, and bootstrap checksums live in
``internal/metadata/bootstrap_sources.json`` and
``internal/metadata/external_sources.json``; they are not duplicated here.
"""
