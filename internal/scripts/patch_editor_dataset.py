...
tables in EXCLUDED_TABLES below) is left byte-for-byte untouched. Each
table's replace is wrapped in its own transaction, so a read failure on one
file rolls that table back rather than leaving it empty -- verified by
testing this script against a real shipped dataset (see
...
def patch(editor_repo: Path, editor_sqlite: Path, output: Path, repo_root: Path) -> None:
...
def main(argv=None) -> int:
    ...
    patch(args.editor_repo.resolve(), args.editor_sqlite.resolve(),
