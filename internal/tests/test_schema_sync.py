...
def test_source_numeric_field_missing_from_our_schema_is_detected():
    header = ["name", "a"]
    schema = {"columns": header, "numeric": []}
    read = _fields(("nm", "character"), ("a", "real"))
    f = schema_sync._classify("x.frt", header, schema, read)
    assert f.kind == schema_sync.TYPE_MISMATCH
    assert "permits text" in f.detail
...
def test_default_artifact_selection_uses_numeric_version_order(tmp_path):
    schema_dir = tmp_path / schema_sync.SCHEMAS_DIR
    schema_dir.mkdir(parents=True)
    for version in ("62.9.0", "62.10.0"):
        artifact = _artifact({}, version=version)
        (schema_dir / f"swatplus-{version}.json").write_text(
            json.dumps(artifact))

    artifact, filename = schema_sync.load_artifact(tmp_path, None)

    assert artifact["swatplus_version"] == "62.10.0"
    assert filename == "swatplus-62.10.0.json"
