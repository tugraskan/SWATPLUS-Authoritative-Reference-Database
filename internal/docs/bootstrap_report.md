# Reference Data Provenance Report
...
Method: serialized directly from the sqlite tables using SWAT+ Editor's own `fileio` writers, so each file is byte-for-byte what SWAT+ Editor writes into a project.

## Files from the official dataset
...
| `pathogens.pth` | editor table ships empty; example/seed values | needs_review |
