...
def test_required_file_cannot_be_marked_unavailable(tmp_path):
    entry = dict(name="plants.plt", status="unavailable", required=True)
    _min_repo(tmp_path, [entry])
    P = vdb.validate(tmp_path)
    assert any("required file cannot have status" in e for e in P.errors)


def test_unmanifested_database_file_is_rejected(tmp_path):
    _min_repo(tmp_path, [], {"typo.ops": "not validated\n"})
    P = vdb.validate(tmp_path)
    assert any("typo.ops" in e and "not listed" in e for e in P.errors)


def test_duplicate_manifest_entry_is_rejected(tmp_path):
    entry = dict(name="plants.plt", status="unavailable", required=True)
    _min_repo(tmp_path, [entry, entry])
    P = vdb.validate(tmp_path)
    assert any("plants.plt" in e and "duplicate manifest" in e for e in P.errors)


def test_nameless_manifest_entry_is_rejected(tmp_path):
    _min_repo(tmp_path, [{"status": "available", "required": False}])
    P = vdb.validate(tmp_path)
    assert any("every manifest entry must have a name" in e for e in P.errors)


def test_manifest_required_flag_must_match_configuration(tmp_path):
    entry = dict(name="plants.plt", status="available", required=False)
    _min_repo(tmp_path, [entry], {"plants.plt": "plants.plt\nname\ncorn\n"})
    P = vdb.validate(tmp_path)
    assert any("does not match EXPECTED_FILES" in e for e in P.errors)
