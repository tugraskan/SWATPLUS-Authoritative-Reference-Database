...
VALID_STATUSES = {"available", "needs_review", "unavailable", "excluded", "deprecated"}
...
def validate(repo_root: str | Path, verify_bootstrap_checksums=False) -> Problems:
    ...
    named_entries = [f for f in manifest_entries
                     if isinstance(f, dict) and f.get("name")]
    if len(named_entries) != len(manifest_entries):
        P.err("database_manifest.json", "every manifest entry must have a name")

    manifest_names = [f["name"] for f in named_entries]
    for name in sorted({n for n in manifest_names if manifest_names.count(n) > 1}):
        P.err(name or "database_manifest.json", "duplicate manifest entry")

    manifest_files = {f["name"]: f for f in named_entries}
    expected_names = set(EXPECTED_BY_NAME)
    actual_names = {p.name for p in db_dir.iterdir()} if db_dir.is_dir() else set()

    for name in sorted(expected_names - set(manifest_files)):
        P.err(name, "expected file is missing from the manifest")
    for name in sorted(set(manifest_files) - expected_names):
        P.err(name, "manifest entry is not defined in EXPECTED_FILES")
    for name in sorted(actual_names - set(manifest_files)):
        P.err(name, "file is not listed in the manifest")

    for name, entry in manifest_files.items():
        status = entry.get("status")
        if status not in VALID_STATUSES:
            P.err(name, f"invalid manifest status {status!r}")
        ...
        if entry.get("required") is not expected_required:
            P.err(name, f"manifest required={entry.get('required')!r} does not "
                        f"match EXPECTED_FILES required={expected_required}")
        if expected_required and status not in ("available", "needs_review"):
            P.err(name, f"required file cannot have status {status!r}")
...
def _check_name_keyed(P: Problems, name: str, path: Path, fmt: str) -> None:
    ...
    if schema and table.columns and table.columns != cols:
        ...
        P.err(name,
              ...
              "the new SWAT+ release and describe it in the pull request.",
    ...
    if table.count_declared is not None \
        ...
        P.err(name, f"declared record count {table.count_declared} != "
...
def _check_decision(P: Problems, name: str, path: Path) -> None:
    ...
    for problem in parsed.problems:
        P.err(name, problem.reason, line=problem.line_no)
    ...
    if not parsed.tables:
        P.err(name, "no decision tables parsed")
    if parsed.declared_count is None:
        P.err(name, "decision-table count is missing")
    elif parsed.declared_count != len(parsed.tables):
        P.err(name, f"declared table count {parsed.declared_count} != "
